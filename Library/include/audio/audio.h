...
#define ENABLE_DMG_RIGHT(ch)                                                   \
  ((1 << 8) << (ch - 1)) // 8-11 sound 1-4 enable flags RIGHT
#define ENABLE_DMG_LEFT(ch)                                                    \
  ((1 << 12) << (ch - 1)) // 12-15 sound 1-4 enable flags LEFT
// add disable macros
...
#define DS_MONO_INIT                                                           \
  (DSA_ENABLE_L | DSA_ENABLE_R | DSB_ENABLE_L | DSB_ENABLE_R |                 \
   DSA_FULL_VOLUME | DSB_FULL_VOLUME | DSA_FIFO_CLEAR | DSB_FIFO_CLEAR)
#define DS_STEREO_INIT                                                         \
  (DSA_ENABLE_L | DSB_ENABLE_R | DSA_FULL_VOLUME | DSB_FULL_VOLUME)
